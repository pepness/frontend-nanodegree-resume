...
bio["display"] = function() {
	...
	var formattedPic = HTMLbioPic.replace("%data%", bio.imgURI);
	...
	var arrayContacs = [formattedMobile, formattedEmail, formattedGitHub, formattedLoc];
...
projects.display = function() {
	...
	for(project in projects.projects) {
		...
		if(projects.projects[project].imgURI.length > 0) {
			for(image in projects.projects[project].imgURI) {
				var formtattedImage = HTMLprojectImage.replace("%data%", projects.projects[project].imgURI[image]);
				//console.log("protam3 : ", projects.projects[project].imgURI.[image]]);
...
education["display"] = function() {
	...
	for(school in education.onlineCourses) {
		(function () {	
			...
			var formattedName = HTMLonlineTitle.replace("%data%", education.onlineCourses[school].title);
			$(".education-entry:last").append(formattedName);

			var formattedDegree = HTMLonlineSchool.replace("%data%", education.onlineCourses[school].school);
			$(".education-entry:last").append(formattedDegree);
			
			var formattedDates = HTMLonlineDates.replace("%data%", education.onlineCourses[school].dates);
			$(".education-entry:last").append(formattedDates);
			
			var formattedLocation = HTMLonlineURL.replace("%data%", education.onlineCourses[school].url);
			$(".education-entry:last").append(formattedLocation);
		}());
	}
